...
export default function Home() {
  ...
  const fetchData = async () => {
    ...
    const querySnapshot = await getDocs(
      query(menuCollection, orderBy("sold", "desc"), limit(4))
    );
    ...
    <div className="py-10 lg:py-20">
      <main className="mt-10 flex lg:flex-row flex-col-reverse items-center">
        <div className="flex flex-col gap-8 lg:w-3/5">
          <h1 className="text-6xl text-center lg:text-left lg:text-8xl text-black font-extrabold max-w-xl">
            TONKUTSU <br />
            RAMEN
          </h1>
          <p className="max-w-lg text-black/70 text-xl">
      ...
      <section className="mt-10 pt-10">
        ...
        <div className="flex flex-wrap gap-2 lg:gap-8">
          {servicesItems.map((item, index) => (
            <div
              key={index}
              className="p-4 gap-3 w-44 lg:w-52 bg-white hover:bg-foreground/10 drop-shadow-lg border rounded-lg flex items-center text-foreground"
            >
              {item.icon}
